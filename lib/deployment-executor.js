...
class DeploymentExecutor {
    ...
    async.series(runningDeployment.products.map((product, idx) => {
      return (done) => {
        this._socket.emit(socketChannel, {
          ...
          deployment: this._mgStepStarted(deployment, idx)
        });
        ...
        const progressHandler = (update) => {
          ...
          this._socket.emit(socketChannel, {
            message: 'progress',
            deployment: this._mgStepProgress(deployment, idx, fixedProgress)
          });
        };
        ...
        const doneHandler = () => {
          ...
          this._socket.emit(socketChannel, {
            message: 'step_done',
            deployment: this._mgStepDone(deployment, idx)
          });
          done(null);
        };
      ...
      this._socket.emit(socketChannel, {
        message: 'deployment_done',
        deployment: this._mgDeploymentDone(deployment)
      });
    });
  ...
  _mgDeploymentDone(deployment) {
    return this._mgStatus(deployment, 'done');
  }
